...
import { ActivitiesItem } from 'index/components/Master/components/deviceEntry/pc/Navigation/ActivitiesItem';
...
describe('index/components/deviceEntry/pc/Navigation/ActivitiesItem', () => {
  ...
  it('The expandButton text should be equal to online_activities_label.', () => {
    ...
    component.setProps({ expandSecondaryMenu: fromJS({ activity: true }) });
    expect(expandButton.text()).toEqual(`Expand: ${context.getWording('online_activities_label')}`);
  });

  it('Should call expandSecondaryMenuAction when enter a char', () => {
    const { expandButton, actions, component } = setup();
    component.setProps({ expandSecondaryMenu: fromJS({ activity: false }) });
