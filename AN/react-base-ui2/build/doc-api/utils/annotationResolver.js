...
const annotationResolver = (ast, recast) => {
  const components = [];
  ...
  const exportTagged = path => {
    ...
    const isTagged = leadingComments.reduce(
      (acc, comment) =>
        acc === true || comment.value.indexOf("@react-component") !== -1,
      [],
    );
    ...
    definitions.forEach(definition => {
      if (definition && components.indexOf(definition) === -1) {
        const superClass = definition.value.superClass;
        components.push({
          node: docgenUtils.resolveToValue(resolveHOC(definition)),
          superClassName: superClass ? superClass.name : null
        });
      }
    });
...
export default annotationResolver;
