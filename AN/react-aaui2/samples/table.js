...
class App extends PureComponent {
  ...
  handleSort = ({ column, order }) => {
    const newCols = _.cloneDeep(this.state.cols)

    newCols[column].sorted = order

    this.setState({ cols: newCols, sortBy: column })
  }

  handlePaging = ({ currentPage }) => {
    const newPagination = _.cloneDeep(this.state.pagination)
  ...
  render() {
    const { dataSource, sortBy, cols, selected, pagination } = this.state

    return (
      <div style={{ fontFamily: 'sans-serif' }}>
        <Table
          textOverflow="hyphenate"
