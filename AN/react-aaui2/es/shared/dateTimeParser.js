...
/**
  ...
  *
*/
export var dateTimeParserConfig = {
  yyyy: [function (v) {
...
export default function dateTimeParser(symbol) {
  return function (value) {
    ...
    var tokens = _Object$keys(dateTimeParserConfig);
    ...
    for (var i = 0; i < tokens.length; i += 1) {
      ...
      if (template.indexOf(token) !== -1) {
        ...
        var reducer = reduceReducers.apply(undefined, dateTimeParserConfig[token]);
        ...
        if (tokenValue) {
          // Handle `a` token
          if (!_Number$isInteger(tokenValue)) {
